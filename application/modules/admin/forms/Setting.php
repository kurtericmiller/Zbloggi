...
class Admin_Form_Setting extends Zend_Form
{
    public function init()
    {
        ...
        $homeArticles = new Zend_Form_Element_Text('homeArticleCount');
        $homeArticles->setLabel('Number of Articles on Home Page:')->setOptions(array('size' => '4'))->setRequired(true)->addValidator('Int')->addFilter('HtmlEntities')->addFilter('StringTrim');
        $articles = new Zend_Form_Element_Text('articleCount');
        $articles->setLabel('Number of Articles per Blog Page:')->setOptions(array('size' => '4'))->setRequired(true)->addValidator('Int')->addFilter('HtmlEntities')->addFilter('StringTrim');
        ...
        $this->addElement($site_title)->addElement($site_tagline)->addElement($site_url)->addElement($headline)->addElement($articles)->addElement($homeArticles)->addElement($books)->addElement($latest)->addElement($amazon);
        ...
        $this->addDisplayGroup(array('site_title', 'site_tagline', 'site_url', 'headlineId', 'homeArticleCount', 'articleCount', 'bookCount', 'latestArticleCount', 'amazonLU',), 'globals', array('legend' => 'Global Settings', 'decorators' => array('FormElements', array('Fieldset', array('class' => 'globalFields')),),));
