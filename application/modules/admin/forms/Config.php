...
class Admin_Form_Config extends Zend_Form
{
  ...
  {
    // initialize form
    $this->setAction('/admin/config')->setMethod('post');
    ...
    $amazon->setLabel('Amazon Lookup Keywords:')->setOptions(array('size' => '14'))->setRequired(true)->addValidator('NotEmpty', true)->addFilter('HtmlEntities')->addFilter('StringTrim');
    ...
    $defemail->setLabel('Default Admin email:')->setOptions(array('size' => '14'))->setRequired(true)->addValidator('NotEmpty', true)->addFilter('HtmlEntities')->addFilter('StringTrim');
    $this->addElement($headline)->addElement($articles)->addElement($books)->addElement($latest)->addElement($amazon);
    ...
    $this->addDisplayGroup(array('headlineId', 'articleCount', 'bookCount', 'latestArticleCount', 'amazonLU',), 'globals', array('legend' => 'Global Settings', 'decorators' => array('FormElements', array('Fieldset', array('class' => 'globalFields')),),));
